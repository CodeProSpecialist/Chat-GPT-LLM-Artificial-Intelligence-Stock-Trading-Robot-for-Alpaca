...
def trading_robot(symbol, X, Y):
    ...
    content = (
        ...
        f"Buy when prices have dropped sharply in the morning and start to recover during the time range: 10:15-11:15. "
        f"Look for stocks that have experienced a pre-market surge ( during the time range: 8:30-9:30), but have since "
        f"pulled back. "
        f"Consider selling or taking profits if a stock has rallied sharply in the morning and continues to rise into "
        f"the afternoon during the time range of: 13:00-14:00. "
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    print("Waiting 12 seconds to not exceed API rate limits and to keep the video card at a colder "
                          "temperature. ")
                    time.sleep(12)  # Add a 1-second delay
